...
import { useTeachers } from "../app/useTeachers";
...
const Teachers = () => {
  const [editingTeachers, setEditingTeachers] = useState<TeachersType | null>(
    null
  );
  ...
  const handleEdit = (teacher: TeachersType) => {
  ...
  const saveEditedTeachers = async () => {
    try {
      if (editingTeachers) {
        // Perform update operation
        await fetch(`http://localhost:3000/teachers/${editingTeachers.id}`, {
          method: "PUT",
          headers: {
  ...
  const { loading, error, teachers, getTeachers } = useTeachers();
              ...
              ? teachers.map((teacher) => (
                  <tbody>
                    <tr className="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                      ...
                      <td className="px-6 py-4">{teacher.group}</td>
                      <td className="px-6 py-4 flex gap-5">
                        <div>
                          {editingTeachers === teacher ? (
                            <button
                              onClick={saveEditedTeachers}
                              type="button"
